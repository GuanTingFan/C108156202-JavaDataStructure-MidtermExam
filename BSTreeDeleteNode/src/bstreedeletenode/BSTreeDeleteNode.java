...
public class BSTreeDeleteNode {      // 二元搜尋樹類別

    public TreeNode head;   // 參考樹的根節點
    // 建立空的二元搜尋樹

    public BSTreeDeleteNode() {
        head = null;
    }
    ...
    //方法: 刪除節點
    public TreeNode deleteNode(TreeNode node, int dVal) {
        //如果大於則往右節點找
        if (node.data < dVal && node.right != null) {
            node.right = deleteNode(node.right, dVal);
            //如果小於則往左節點找
        } else if (node.data > dVal && node.left != null) {
            node.left = deleteNode(node.left, dVal);
            //相等代表找到該值
        } else {
    ...
    public static void main(String args[]) {
        int i;
        int[] array = {11, 3, 20, 15, 33, 16};

        BSTreeDeleteNode obj = new BSTreeDeleteNode();
        ...
        obj.head = obj.deleteNode(obj.head, 20);
